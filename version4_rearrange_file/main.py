    ...
    try:
        ...
        if record.id:
            cursor.execute('''
                ...
                WHERE id=%s
            ''', (record.documentNumber, record.date, record.customer_name, record.customer_code, record.expectedQty, record.actualQty, record.checkerName, pallet_details_json, record.imageBase64, record.plant_ticketcode, total_calculated_weight, record.id))
        else:
            cursor.execute('''
                INSERT INTO receipt_data (document_no, date, customer_name, customer_code, expected_qty, actual_qty, checker_name, pallet_details, image_base64, plant_ticketcode, calculated_weight) 
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ''', (record.documentNumber, record.date, record.customer_name, record.customer_code, record.expectedQty, record.actualQty, record.checkerName, pallet_details_json, record.imageBase64, record.plant_ticketcode, total_calculated_weight))
